...
(defui posts
  ...
  (let [query-client (useQueryClient)
        query        (useQuery
                      #js {:queryKey #js ["posts"]
                           :queryFn  #(-> (js/fetch "https://jsonplaceholder.typicode.com/posts")
                                          (.then (fn [res] (.json res))))})]
    ($ :<>
       (cond (.-isLoading query) "Loading..."
             (.-error query) "Error while loading posts!"
             :default (str (count (.-data query))
